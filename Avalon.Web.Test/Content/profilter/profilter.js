...
$profiler =
{
    ...
    _createItem: function (item) {
        ...
        //html.push('<span class="profiler-tabbutton" data-type="profiler">Profiler (' + item.Profilers.length + ')</span>');
        html.push('</div>');
        ...
        html.push('<div class="profiler-tabbody profiler-tabbody-profiler">');
        html.push('<div class="profiler-header"><strong>时间线</strong></div>');
        //for (var i = 0; i < item.Profilers.length; i++) {
        //    this._createProfileItem(html, item.Profilers[i], 0);
        //}
        html.push('</div>');
        ...
        e.find(".profiler-item span").click($.proxy(this._onProfilerItemClick, this));
    },
    _createProfileItem: function (html, pi, depth) {
        html.push('<div class="profiler-item"><strong>' + pi.Start + '</strong><span style="padding-left:' + (12 + depth * 24) + 'px;">[' + pi.Duration + '] ' + this._createMessage(pi.Message) + '</span></div>');
        html.push('<div style="display: none;"><pre>' + pi.StackTrace + '</pre></div>');
        if (pi.Items.length > 0)
            for (var i = 0; i < pi.Items.length; i++)
                this._createProfileItem(html, pi.Items[i], depth + 1);
    },

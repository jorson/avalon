 ...
 $profiler = 
{
    ...
    _createItem : function(item){
        ...
        for(var t in types)
            html.push('<span data-type="' + t + '" class="profiler-trace-type-active">' + t + '(' + types[t] + ')</span>');
        html.push('</div>');

        html.push('<dl>');
        for(var i = 0; i < item.Traces.length; i ++ ){
            var ti = item.Traces[i];
            html.push('<dt data-type="' + ti.Type + '"><strong>' + ti.Type + '</strong><pre class="' + this._getLang(ti.Type) + '">' + ti.Title + '</pre></dt>');
        ...
        for(var i = 0; i < item.Profilers.length; i ++ ){
            var pi = item.Profilers[i];
            html.push('<dt>' + pi.Title + '</dt>');
            html.push('<dd>' + pi.Content + '</dd>');
        }
        html.push('</dl>');
        html.push('</div>');

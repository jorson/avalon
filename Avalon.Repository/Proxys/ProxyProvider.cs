...
namespace Avalon.Framework
{
    public static class ProxyProvider
    {
        ...
        //            Fetch(entity, metadata.FetchableObject.CheckFetchStop);
        //        }
        //        else
        //        {
        //            Fetch(entity, null);
        ...
        public static void Fetch<T>(T entity)
        {
            var metadata = RepositoryFramework.GetDefineMetadata(typeof(T));
            if (metadata == null || metadata.FetchableObject == null)
                throw new AvalonException("类型 {0} 不支持 Fetch 的操作。", typeof(T).FullName);
            metadata.FetchableObject.Fetch(entity);
        }

        public static void FetchList<T>(IEnumerable<T> entities)
        {
            var metadata = RepositoryFramework.GetDefineMetadata(typeof(T));
            if (metadata == null || metadata.FetchableObject == null)
                throw new AvalonException("类型 {0} 不支持 FetchList 的操作。", typeof(T).FullName);

            metadata.FetchableObject.FetchList(entities);
        }

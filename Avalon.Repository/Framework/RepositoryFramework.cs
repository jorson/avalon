...
using Avalon.Framework.Configurations;
...
namespace Avalon.Framework
{
    ...
    public static class RepositoryFramework
    {
        ...
        public static IQuerySourceProvider QuerySourceProvider
        {
            get { return QueryMetadataProvider.SourceProvider; }
            set { QueryMetadataProvider.SourceProvider = value; }
        }

        public static IQuerySpecificationProvider QuerySpecificationProvider
        {
            get { return QueryMetadataProvider.SpecificationProvider; }
            set { QueryMetadataProvider.SpecificationProvider = value; }
        }

        public static IQuerySpecification<TFilter> CreateQuerySpecification<TFilter>()
        {
            frameworkConfiguation.CheckInited();
            return QuerySpecificationProvider.CreateSpecification<TFilter>();
        }
        ...
        {
            frameworkConfiguation.Configure(new DefaultDependency());
        }

        public static void RegisterRepositoryAssemblies(params Assembly[] assemblies)
        {
            frameworkConfiguation.RegisterRepositoryAssemblies(assemblies);
        }

        public static void RegisterServiceAssemblies(params Assembly[] assemblies)
        {
            frameworkConfiguation.RegisterServiceAssemblies(assemblies);
        }
        ...
        public static ClassDefineMetadata GetDefineMetadataAndCheck(Type entityType)
        {
            ...
            if (metadata == null)
                throw new AvalonException("无法获取对象的定义 {0}", entityType.FullName);
        ...
        public static void RaisePersistent(object entity)
        {
            ...
            if (metadata == null)
                throw new AvalonException("无法获取对象的定义 {0}", entity.GetType().FullName);
            ...
            if (depends.Count > 0)
            {
                ...
                var items = depends.Select(o => new CacheItem(o, time)).ToList();
        ...
        public static TJoin GetJoin<TJoin>(object entity, Expression<Func<TJoin>> memberExpr)
        {
            ...
            if (joinDefine == null)
                throw new AvalonException("类型 {1} 方法 {0} 未进行关联的定义。", method.Name, method.DeclaringType.FullName);

            return (TJoin)joinDefine.DataProcesser.Process(entity);
        }

        public static void Fetch<T>(T entity) where T : class
        {
            ProxyProvider.Fetch(entity);
        }

        public static void FetchList<T>(IEnumerable<T> entities) where T : class
        {
            ProxyProvider.FetchList(entities);
        }
        ...
        internal static ICache GetCacher(ClassDefineMetadata metadata)
        {
            ...
            return CacheManager.GetCacher(metadata.EntityType);
        }
        ...
        internal static ICache GetCacher(ClassDefineMetadata metadata, string name)
        {
            ...
            return CacheManager.GetCacher(name);
        }
